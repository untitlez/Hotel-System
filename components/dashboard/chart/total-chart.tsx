...
interface TotalChartProps {
  rooms: ResponseRoomType[];
  bookings: ResponseBookingType[];
}

export function TotalChart({ rooms, bookings }: TotalChartProps) {
  ...
  const groupedData = bookings.reduce((acc, booking) => {
    ...
    rooms.map((room) => {
      const key = `${date}-${room.type}`;
      acc[key] = {
        date,
        type: room.type,
        price: 0,
      };
      acc[key].price += room.pricePerNight;
    });
    return acc;
  }, {} as Record<string, { date: string; type: string; price: number }>);
